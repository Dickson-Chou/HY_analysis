...
//  TH1F* YMass = new TH1F("YMass","",2,0,2);
  ...
  vector<TH1F*> YPN(16);
  vector<TH1F*> YPN_d(16);
  YPN[0] = new TH1F("YPN","",100,0,1);
  
  for(int i = 0 ; i < 16 ; i++){
    YPN[i] = (TH1F*)YPN[0]->Clone(Form("signal_%4.0f",YM[i]));
    YPN_d[i] = (TH1F*)YPN[0]->Clone(Form("signal_d_%4.0f",YM[i]));
  }
...
*/	
    ...
    if(100 < FatJet_msoftdrop[leadingID] && FatJet_msoftdrop[leadingID] < 140 && 160 < FatJet_msoftdrop[subleadingID]){
      M_H.push_back(FatJet_msoftdrop[leadingID]);
      M_Y.push_back(FatJet_msoftdrop[subleadingID]);
      nGoodPairJet++;
      masspass = true;
      if(FatJet_pt[leadingID] > 300 && FatJet_pt[subleadingID] > 200){
//        I_FatJet_ParticleNetMD_ProbXbb = FatJet_ParticleNetMD_probXbb[subleadingID] ; 
//        YPN->Fill(FatJet_ParticleNetMD_probXbb[subleadingID]); 
        for(int iymass=0; iymass < nYmass; iymass++){
          if(data.GetBool(YmassNames[iymass].data())==true){
            YPN[iymass]->Fill(FatJet_ParticleNetMD_probXbb[subleadingID]);
          }
        }
...
/*      if(FatJet_ParticleNetMD_probXbb[subleadingID] > 0.85){
        ...
        HDeep_N->Fill(FatJet_deepTagMD_ZHbbvsQCD[subleadingID]);
      }
*/      
    

    if(100 < FatJet_msoftdrop[subleadingID] && FatJet_msoftdrop[subleadingID] < 140 && 160 < FatJet_msoftdrop[leadingID]){
      M_H.push_back(FatJet_msoftdrop[subleadingID]);
      M_Y.push_back(FatJet_msoftdrop[leadingID]);
      nGoodPairJet++;
      masspass = true;
      if(FatJet_pt[leadingID] > 200 && FatJet_pt[subleadingID] > 300){
//        I_FatJet_ParticleNetMD_ProbXbb = FatJet_ParticleNetMD_probXbb[leadingID] ; 
//        YPN->Fill(FatJet_ParticleNetMD_probXbb[leadingID]);
        for(int iymass=0; iymass < nYmass; iymass++){
          if(data.GetBool(YmassNames[iymass].data())==true){
            YPN[iymass]->Fill(FatJet_ParticleNetMD_probXbb[leadingID]);
          }
        }
...
/*
      if(FatJet_ParticleNetMD_probXbb[leadingID] > 0.85){
        ...
        HDeep_N->Fill(FatJet_deepTagMD_ZHbbvsQCD[leadingID]);
      }
*/      
...
/*
    ...
    */
//    YMass->Fill(GenModel_YMass_150.data());
    variable->Fill();
